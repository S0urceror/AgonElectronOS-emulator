...
static VIDEO_MODES: [VideoMode; 4] = [VideoMode{colors: 2, screen_width: 1024, screen_height: 768, refresh_rate: 60, palette: &PALETTE_2},
                                    VideoMode{colors: 16, screen_width: 512, screen_height: 384, refresh_rate: 60, palette: &PALETTE_16}, // default mode
...
fn msxcolorconvert (color:u8) -> sdl2::pixels::Color {
    /*
    ...
    const RGB888 COLOR2RGB888[16] = {
        ...
        {   0, 128, 128 }, // Cyan              6
        { 128, 128, 128 }, // White             7
        {  64,  64,  64 }, // BrightBlack       8
        { 255,   0,   0 }, // BrightRed         9
        {   0, 255,   0 }, // BrightGreen       10  
        { 255, 255,   0 }, // BrightYellow      11
        {   0,   0, 255 }, // BrightBlue        12
        { 255,   0, 255 }, // BrightMagenta     13
        {   0, 255, 255 }, // BrightCyan        14
        { 255, 255, 255 }, // BrightWhite       15
    };
    */
    match color {
        1 => {*PALETTE_16[0]} // black
        2 => {*PALETTE_16[2]} // green
        3 => {*PALETTE_16[10]} // lightgreen
        4 => {*PALETTE_16[4]} // blue
        5 => {*PALETTE_16[12]} // light blue
        6 => {*PALETTE_16[1]} // brown
        7 => {*PALETTE_16[6]} // cyan
        8 => {*PALETTE_16[1]} // red
        9 => {*PALETTE_16[9]} // light red
        10 => {*PALETTE_16[3]} // yellow
        11 => {*PALETTE_16[11]} // light yellow
        12 => {*PALETTE_16[2]} // dark green
        13 => {*PALETTE_16[5]} // purple
        14 => {*PALETTE_16[8]} // grey
        _ => {*PALETTE_16[color as usize]}
    }
}                                    
...
impl VDP<'_> {
    pub fn new(canvas: Canvas<Window>, texture_creator: &TextureCreator<WindowContext>, tx: Sender<u8>, rx: Receiver<u8>, vsync_counter: std::sync::Arc<std::sync::atomic::AtomicU32>, audio_subsystem: AudioSubsystem) -> Result<VDP, String> {
        ...
        Ok({
            ...
            cmd_bytes: 0,
            command: Vec::new(),
            tms9918: TMS9918 {registers: [0;8],memory: [0; 16*1024],port99_write: false,data_register:0,
                              read_address:0,write_address:0,status_register:0,mode:1,
                              mem16kb:true,screen_enable:true,interrupt_enable:true,sprite_size_16:false,sprite_magnify:false,
                              nametable:0,colortable:0,patterntable:0,sprite_attribute_table:0,sprite_pattern_table:0,textmode_colors:0},
            };
            for _ in 0..256 {
                v.bitmaps.push(None);
            };
...
impl VDP<'_> {
    ...
    fn bootscreen(&mut self) {
        let boot_message = "Electron - HAL - version 0.1.0";
    ...
    fn render_msx_screen (&mut self) {
        match self.tms9918.mode {
            0b100 => {
                self.canvas.with_texture_canvas(&mut self.texture, |texture_canvas| {
                    let mut patterntable:u16 = 0x0000;
                    let mut colortable:u16 = 0x0000;

                    // in graphics II only
                    if self.tms9918.patterntable & 0b0010_0000_0000_0000 > 0 {
                        patterntable = 0x2000;
                    }
                    if self.tms9918.colortable & 0b0010_0000_0000_0000 > 0 {
                        colortable = 0x2000;
                    }

                    // check all 768 entries in the nametable
                    for i in 0..768 {
                        let mut ptX = (i % 32) * 8;
                        let mut ptY = (i/32) * 8;
                        let mut patsection:u8 = (i / 256) as u8;
                        // get 8x8 pixels
                        let patindex = self.tms9918.memory[(self.tms9918.nametable+i as u16) as usize];
                        for y in 0..=7 {
                            // which pattern belongs to this index?
                            let mut pataddr = (patterntable+
                                               (patindex as u16)*8+
                                               (patsection as u16 * 0x800)+
                                               y as u16);
                            let mut patbyte:u8 = self.tms9918.memory[pataddr as usize];
                            // which color belongs to this index?
                            let mut clraddr = (colortable+
                                               (patindex as u16)*8+
                                               (patsection as u16 * 0x800)+
                                               y as u16);
                            let mut clrbyte:u8 = self.tms9918.memory[clraddr as usize];
                            // draw 2x2 pixels
                            for x in 0..=7 {
                                if patbyte & (1 << (7-x)) != 0 {
                                    // draw foreground color
                                    let mut foregroundcolor:u8 = (clrbyte & 0xf0) >> 4;
                                    let mut color = msxcolorconvert (foregroundcolor);
                                    //let mut color = *PALETTE_16[foregroundcolor as usize];
                                    texture_canvas.set_draw_color(color);
                                    //texture_canvas.draw_point(pt.offset (x,y));
                                    let mut rect = Rect::new ((ptX+x)*2,(ptY+y)*2,2,2);   
                                    texture_canvas.fill_rect(rect);
                                } else {
                                    // draw background color
                                    let mut backgroundcolor:u8 = clrbyte & 0x0f;
                                    let mut color = msxcolorconvert (backgroundcolor);
                                    //let mut color = *PALETTE_16[backgroundcolor as usize];
                                    texture_canvas.set_draw_color(color);
                                    let mut rect = Rect::new ((ptX+x)*2,(ptY+y)*2,2,2);                                    
                                    texture_canvas.fill_rect(rect);
                                }
    ...
    fn process_command (&mut self) {
        match self.command[0] {
            0x00 => {
                //eprintln! ("OUT ({:02X}),{:02X}",self.command[1],self.command[2]);
                if self.command[1]==0x98 {
                    //eprintln! ("VRAM {:04X} < {:02X}",self.tms9918.write_address,self.command[2]);
                    self.tms9918.memory[self.tms9918.write_address as usize]=self.command[2];
                    self.tms9918.write_address = (self.tms9918.write_address + 1) & 0x3fff;
                }
                if self.command[1]==0x99 {
                    if self.tms9918.port99_write == false {
                        self.tms9918.port99_write = true;
                        self.tms9918.data_register = self.command[2];
                    } else {
                        self.tms9918.port99_write = false;
                        // write register
                        if self.command[2] & 0b11000000 == 0b10000000 {
                            let mut reg = self.command[2] & 0b00000111;
                            // store raw info
                            self.tms9918.registers[reg as usize]=self.tms9918.data_register;
                            //eprintln! ("VDP({:02X}) < {:02X}",reg,self.tms9918.data_register);
                            // extract info
                            match reg {
                                0 => {
                                    let mut mode = self.tms9918.data_register & 0b00001110;
                                    mode = mode << 1;
                                    let mut oldmode = self.tms9918.mode;
                                    self.tms9918.mode = self.tms9918.mode & 0b00000011;
                                    self.tms9918.mode = self.tms9918.mode | mode;
                                    if (oldmode!=self.tms9918.mode) {
                                        match self.tms9918.mode {
                                            0b000 => {
                                                eprintln!("Graphics I Mode");
                                                self.cursor_enabled=false;
                                            },
                                            0b100 => {
                                                eprintln!("Graphics II Mode");
                                                self.cursor_enabled=false;
                                            },
                                            0b010 => {
                                                eprintln!("Multicolor Mode");
                                                self.cursor_enabled=false;
                                            },
                                            0b001 => {
                                                eprintln!("Text Mode");
                                ...
                                1 => {
                                    let mut mode = self.tms9918.data_register & 0b00011000;
                                    mode = mode >> 3;
                                    mode = mode & 1 << 1 + mode & 2 >> 1; // rotate bits
                                    let mut oldmode = self.tms9918.mode;
                                    self.tms9918.mode = self.tms9918.mode & 0b00011100;
                                    self.tms9918.mode = self.tms9918.mode | mode;
                                    if (oldmode!=self.tms9918.mode) {
                                        match self.tms9918.mode {
                                            0b000 => {
                                    ...
                                    self.tms9918.mem16kb = self.tms9918.data_register & 0b10000000 != 0;
                                    self.tms9918.screen_enable = self.tms9918.data_register & 0b01000000 != 0;
                                    self.tms9918.interrupt_enable = self.tms9918.data_register & 0b00100000 != 0;
                                    self.tms9918.sprite_size_16 = self.tms9918.data_register & 0b00000010 != 0;
                                    self.tms9918.sprite_magnify = self.tms9918.data_register & 0b00000001 != 0;
                                }
                                2 => {
                                    let mut nametable:u16 = self.tms9918.data_register as u16;
                                    nametable = nametable << 10;
                                    nametable = nametable & 0x3fff;
                                    self.tms9918.nametable = nametable;
                                    eprintln!("nametable: {:04X}",nametable);
                                }
                                3 => {
                                    let mut colortable:u16 = self.tms9918.data_register as u16;
                                    colortable = colortable << 6;
                                    colortable = colortable & 0x3fff;
                                    self.tms9918.colortable = colortable;
                                    ...
                                    eprintln!("colortable: {:04X}",colortable);
                                }
                                4 => {
                                    let mut patterntable:u16 = self.tms9918.data_register as u16;
                                    patterntable = patterntable << 11;
                                    patterntable = patterntable & 0x3fff;
                                    self.tms9918.patterntable = patterntable;
                                    ...
                                    eprintln!("patterntable: {:04X}",patterntable);
                                }
                                5 => {
                                    let mut sprite_attribute_table:u16 = self.tms9918.data_register as u16;
                                    sprite_attribute_table = sprite_attribute_table << 7;
                                    sprite_attribute_table = sprite_attribute_table & 0x3fff;
                                    self.tms9918.sprite_attribute_table = sprite_attribute_table;
                                    eprintln!("sprite_attribute_table: {:04X}",sprite_attribute_table);
                                }
                                6 => {
                                    let mut sprite_pattern_table:u16 = self.tms9918.data_register as u16;
                                    sprite_pattern_table = sprite_pattern_table << 11;
                                    sprite_pattern_table = sprite_pattern_table & 0x3fff;
                                    self.tms9918.sprite_pattern_table = sprite_pattern_table;
                                    eprintln!("sprite_pattern_table: {:04X}",sprite_pattern_table);
                                }
                                7 => {
                                    self.tms9918.textmode_colors = self.tms9918.data_register;
                                }
                                _ => {
                        ...
                        // vram write address
                        if self.command[2] & 0b11000000 == 0b01000000 {
                            let mut temp:u16 = (self.command[2] & 0b00111111) as u16;
                            temp = temp << 8;
                            self.tms9918.write_address = temp + self.tms9918.data_register as u16;
                            //eprintln! ("VDP write address = {:04X}",self.tms9918.write_address);
                        }
                        // vram read address
                        if self.command[2] & 0b11000000 == 0b00000000 {
                            let mut temp:u16 = (self.command[2] & 0b00111111) as u16;
                            temp = temp << 8;
                            self.tms9918.read_address = temp + self.tms9918.data_register as u16;
                            //eprintln! ("VDP read address = {:04X}",self.tms9918.read_address);
                        }
                    }
                }
            }
            0x01 => {
                let mut val:u8=0;
                if self.command[1]==0x98 {
                    val = self.tms9918.memory[self.tms9918.read_address as usize];
                    //eprint! ("VRAM {:04X} > {:02X}",self.tms9918.read_address,val);
                    self.tms9918.read_address = (self.tms9918.read_address + 1) & 0x3fff;
                }
                if self.command[1]==0x99 {
                    val = self.tms9918.status_register;
                    //eprint! ("VDP (0) > {:02X}",val);
                }
                self.tx.send(self.command[0]|0x80); // repeat command
                self.tx.send(val);                    // value
                //eprintln! (" => {:02X}",val);
            }
            _ => {
    ...
    fn do_comms(&mut self) {
        match self.try_read_byte() {
            Ok(mut n) => {
                if (self.cmd_bytes==0) && (n & 0x80) > 0 {
                    n = n & 0x7f;
                    match n {
                        0x00 => {self.cmd_bytes=3;}
                        0x01 => {self.cmd_bytes=2;}
                        _ => {
                ...
                if self.cmd_bytes > 0 {
                    self.command.push (n);
                    self.cmd_bytes=self.cmd_bytes-1;
                    if self.cmd_bytes == 0 {
                        // command complete
